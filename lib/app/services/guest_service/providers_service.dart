...
import '../../models/meeting_times_model.dart';
...
class ProvidersService {
  static Future<List<UserModel>> getInstructors(
      {List<CategoryModel> categories = const [],
      String? sort,
      bool availableForMeetings = false,
      bool freeMeetings = false,
      bool discount = false,
      bool downloadable = false}) async {
    List<UserModel> data = [];
    try {
  ...
  static Future<List<UserModel>> getOrganizations(
      {List<CategoryModel> categories = const [],
      String? sort,
      bool availableForMeetings = false,
      bool freeMeetings = false,
  ...
  static Future<List<UserModel>> getConsultations(
  ...
  static Future<ProfileModel?> getUserProfile(int id) async {
    try {
      ...
      if (jsonRes['success']) {
        return ProfileModel.fromJson(jsonRes['data']['user'],
  ...
  static Future<bool> follow(int id, bool state) async {
    try {
      ...
      Response res =
          await httpPostWithToken(url, {'status': state ? '1' : '0'});
  ...
  static Future<bool> reserveMeeting(int timeId, String date,
      String meetingType, int studentCount, String description) async {
    try {
  ...
  static Future<bool> sendMessage(
      int userId, String subject, String email, String description) async {
    try {
      String url = '${Constants.baseUrl}users/$userId/send-message';

      Response res = await httpPostWithToken(
          url, {"title": subject, "email": email, "description": description});
