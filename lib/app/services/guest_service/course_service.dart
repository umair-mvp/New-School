...
static Future<SingleContentModel?> getSingleContent(String url) async {
  try {
    ...
    if(jsonResponse['success'] ?? false){
      ...
      print('Raw data: ${jsonEncode(jsonResponse['data'])}');
      
      try {
        return SingleContentModel.fromJson(jsonResponse['data']);
      } catch (parseError) {
        print('Error parsing SingleContentModel: $parseError');
        print('Data that caused error: ${jsonEncode(jsonResponse['data'])}');
        return null;
      }
    } else {
    ...
    print('Exception in getSingleContent: $e');
