...
class UserService {
    ...
    try {
      ...
      if (jsonResponse['success']) {
        locator<UserProvider>()
            .setProfile(ProfileModel.fromJson(jsonResponse['data']['user']));
  ...
  static Future<DashboardModel?> getDashboardData() async {
    try {
      ...
      var jsonResponse = jsonDecode(res.body);
      if (jsonResponse['success'] ?? true) {
        print("DAshboard data: ${jsonResponse['data']}");
  ...
  static Future<bool> updateImage(
    ...
    try {
      ...
      dio.FormData body = dio.FormData.fromMap({
        if (profile != null) ...{
          "profile_image": await dio.MultipartFile.fromFile(profile.path,
              filename: profile.path.split('/').last),
        },
        if (indentity != null) ...{
          "identity_scan": await dio.MultipartFile.fromFile(indentity.path,
              filename: indentity.path.split('/').last),
        },
        if (certificate != null) ...{
          "certificate": await dio.MultipartFile.fromFile(certificate.path,
              filename: certificate.path.split('/').last),
        },
      });

      dio.Response res = await dioPostWithToken(url, body);
